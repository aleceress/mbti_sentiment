...
def get_trait_types(trait):
    TYPES = ["infj", "intj", "infp", "intp", "istj", "isfj", "istp", "isfp", "enfj", "enfp", "entp", "entj", "estj", "esfj", "estp", "esfp"]
...
def get_types_avg_emotion(types):
    types_sum = dict({"love": 0, "joy": 0, "anger": 0, "sadness": 0, "surprise": 0, "fear": 0})

    for personality in types:
        personality_posts = get_posts(personality)
        personality_avg = get_emotions_avg(get_posts_emotions(personality_posts), len(personality_posts))
